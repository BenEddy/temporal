...
func (m *NamespaceConfig) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.WorkflowRules) > 0 {
		for k := range m.WorkflowRules {
			v := m.WorkflowRules[k]
			baseI := i
...
func (m *NamespaceConfig) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 9:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field WorkflowRules", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.WorkflowRules == nil {
				m.WorkflowRules = make(map[string]*v12.WorkflowRule)
			}
			var mapkey string
			var mapvalue *v12.WorkflowRule
			for iNdEx < postIndex {
				entryPreIndex := iNdEx
				var wire uint64
				for shift := uint(0); ; shift += 7 {
			...
			m.WorkflowRules[mapkey] = mapvalue
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
